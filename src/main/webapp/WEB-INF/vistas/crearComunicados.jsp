...
<div class="formularioP border border-dark border-2" style="display: flex; justify-content: center;  background-color: #c1e1c5; margin-top: 30px; padding: 20px;  width: 80%; margin: 0 auto;">
    <form action="/Portafolio/crearComunicados" method="post" onsubmit="return enviarFormulario(event)">
        ...
        <input type="date" id="fecha" name="fecha" class="form-control border border-dark" title="Campo Obligatorio"><br> <br>
        ...
        <input type="text" id="titulo" name="titulo" class="form-control border border-dark" title="Campo Obligatorio"><br> <br>
        <span id="nombreValidationMessage" style="color: red;"></span>
        
        <label for="detalle" class="form-label" style="font-weight: bold;">Ingrese Descripción:</label><br>
        <input type="text" id="detalle" name="detalle" class="form-control border border-dark" title="Campo Obligatorio"><br> <br>
        <span id="nombreValidationMessage" style="color: red;"></span>
...
function cargarAlumnos() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            var response = xhr.response; // Obtenemos directamente la respuesta

            actualizarListaAlumnos(response);
        }
    };
    xhr.send();
}
...
function actualizarListaAlumnos(alumnos) {
    ...
    listaEstudiantesDiv.innerHTML = ""; // Limpiar la lista existente
